...
def get_day_stats():
    with stats_lock:
        ...
        if day not in all_stats:
            all_stats[day] = default_day_stats()
            save_json_file(STATS_FILE, all_stats)
        else:
            merged = default_day_stats()
            merged.update(all_stats[day])
            if "today_users" not in merged or not isinstance(merged["today_users"], list):
                merged["today_users"] = []
            if "usage_by_user" not in merged or not isinstance(merged["usage_by_user"], dict):
                merged["usage_by_user"] = {}
            all_stats[day] = merged
            save_json_file(STATS_FILE, all_stats)
        return all_stats[day]
...
def update_day_stats(endpoint_name, key=None, valid_user=False, successful=False):
    with stats_lock:
        all_stats = load_json_file(STATS_FILE)
        day = today_key()
        day_stats = all_stats.get(day, default_day_stats())

        day_stats["total_requests"] = int(day_stats.get("total_requests", 0)) + 1

        if endpoint_name == "ask":
            day_stats["ask_requests"] = int(day_stats.get("ask_requests", 0)) + 1
        elif endpoint_name == "models":
            day_stats["models_requests"] = int(day_stats.get("models_requests", 0)) + 1
        elif endpoint_name == "keyGenerate":
            day_stats["key_generate_requests"] = int(day_stats.get("key_generate_requests", 0)) + 1

        if successful:
            day_stats["successful_requests"] = int(day_stats.get("successful_requests", 0)) + 1

        if valid_user and key:
            users = day_stats.get("today_users", [])
            if key not in users:
                users.append(key)
            day_stats["today_users"] = users

            usage_map = day_stats.get("usage_by_user", {})
            ...
            day_stats["usage_by_user"] = usage_map

        all_stats[day] = day_stats
        save_json_file(STATS_FILE, all_stats)
...
def check_key_status(key):
    ...
    try:
        expires_at = parse_dt(key_data["expires_at"])
    except Exception:
        return {
            "type": "temporary",
            "valid": False,
